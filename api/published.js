// api/published.js — GET/PUT published snapshot (manager writes, any authed user reads)
// Storage: Vercel KV (Upstash Redis REST API) — key "pcom:published"
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      console.error('[published GET]', err);
      return res.status(500).json({ error: 'Failed to read published snapshot: ' + err.message });
    }
  }

  if (req.method === 'PUT') {
    if (user.role !== 'manager') {
      return res.status(403).json({ error: 'Forbidden — manager role required to publish' });
    }
    const body = req.body;
    if (!body || !body.projects || !body.mbs || !body.pool) {
      return res.status(400).json({ error: 'Invalid snapshot: must include projects, mbs, pool' });
    }
    try {
      await kvSet(KV_KEY, body);
      return res.status(200).json({ ok: true });
    } catch (err) {
      console.error('[published PUT]', err);
      return res.status(500).json({ error: 'Failed to write published snapshot: ' + err.message });
    }
  }
